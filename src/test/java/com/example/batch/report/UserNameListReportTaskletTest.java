...
class UserNameListReportTaskletTest {
    ...
    @Mock
    private UserNameListReportFactory userNameListReportFactory;

    @Mock
    private UserNameListReportHtmlRenderer userNameListReportHtmlRenderer;

    @Mock
    private UserNameListPdfGenerator userNameListPdfGenerator;

    @Mock
    private UserNameListReportFileWriter userNameListReportFileWriter;

    @Mock
    private SftpClient sftpClient;

    @Mock
    private UserNameListReportMessageReader userNameListReportMessageReader;
    ...
    void execute() {
        ...
        UserNameListReportRequestMessage requestMessage =
                new UserNameListReportRequestMessage(
                        "request-1",
                        List.of("user-1", "user-2")
                );
        List<UserResponse> userResponses = List.of(
                new UserResponse("user-1", "山田太郎"),
                new UserResponse("user-2", "佐藤花子")
        );
        ...
        when(userNameListReportMessageReader.receive())
                .thenReturn(Optional.of(requestMessage));
        when(userApiClient.listByIds(List.of("user-1", "user-2")))
                .thenReturn(userResponses);
        when(userNameListReportFactory.create(userResponses))
                .thenReturn(report);
        when(userNameListReportHtmlRenderer.render(report))
                .thenReturn("<html>report</html>");
        when(userNameListPdfGenerator.generate("<html>report</html>"))
                .thenReturn("%PDF".getBytes());
        when(userNameListReportFileWriter.write(pdf))
